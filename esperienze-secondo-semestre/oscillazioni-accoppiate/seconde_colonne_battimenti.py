...
xA, yA = np.loadtxt(r'C:\Users\ACER\OneDrive\Desktop\Laboratorio I\esperienze-secondo-semestre\oscillazioni-accoppiate\DATI_oscilalzioni_accoppiate\oscillazioni_battimenti2_MarcoNico.txt', usecols=(2, 3), unpack=True)
...
#plot del grafico
fig=plt.figure('battimenti seconde colonne')
ax1, ax2 = fig.subplots(2, 1, sharex=True, gridspec_kw=dict(height_ratios=[2, 1], hspace=0.05))

#dimezzamento della lumghezza degli array
xA=xA[::2]
yA=yA[::2]
dxA=dxA[::2]
dyA=dyA[::2]

ax1.errorbar(xA, yA, dyA, dxA, fmt='.', label='Dati', color='blue')

#xgrid = np.linspace(min(xA), max(xA), 1490)
ax1.plot(xA, battimenti1(xA, *popt), label='Modello di best-fit', color='red')
...
ax2.errorbar(xA, res, dyA, fmt='.', color='blue')

ax2.plot(xA, np.full(xA.shape, 0.0), color='red')
